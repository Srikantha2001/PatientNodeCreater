...
startDockerContainer() {
  ...
  if [ "$CCAAS_DOCKER_RUN" = "true" ]; then
    ...
    set -x
    ${CONTAINER_CLI} run --rm -d --name peer${PEER_PATIENT_NUMBER}org1_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest

    ${CONTAINER_CLI} run  --rm -d --name peer${PEER_PATIENT_NUMBER}org2_${CC_NAME}_ccaas \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest
    ...
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer${PEER_PATIENT_NUMBER}org1_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer${PEER_PATIENT_NUMBER}org2_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
...
## Install chaincode on peer${PEER_PATIENT_NUMBER}.org1 and peer${PEER_PATIENT_NUMBER}.org2
infoln "Installing chaincode on peer${PEER_PATIENT_NUMBER}.org1..."
installChaincode 1
infoln "Install chaincode on peer${PEER_PATIENT_NUMBER}.org2..."
installChaincode 2
