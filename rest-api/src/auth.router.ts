...
authRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (
      email === env.get('EMAIL_PATORG').asString() &&
      password === env.get('PASSWORD_PATORG').asString()
    ) {
      return res.status(OK).json({
        status: getReasonPhrase(OK),
        x_api_key: env.get('PATORG_APIKEY').asString(),
        role: 'patorg_admin',
      });
    } else if (
      email === env.get('EMAIL_DOCORG').asString() &&
      password === env.get('PASSWORD_DOCORG').asString()
    ) {
      return res.status(OK).json({
        status: getReasonPhrase(OK),
        x_api_key: env.get('DOCORG_APIKEY').asString(),
        role: 'docorg_admin',
      });
    } else {
