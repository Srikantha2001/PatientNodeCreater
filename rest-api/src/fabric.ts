...
/**
 * Creates an in memory wallet to hold credentials for an PatOrg user
 *
...
export const createWallet = async (): Promise<Wallet> => {
  ...
  const patorgIdentity = {
    credentials: {
      certificate: config.certificatePatOrg,
      privateKey: config.privateKeyPatOrg,
    },
    mspId: config.mspIdPatOrg,
    type: 'X.509',
  };

  await wallet.put(config.mspIdPatOrg, patorgIdentity);

  const docorgIdentity = {
    credentials: {
      certificate: config.certificateDocOrg,
      privateKey: config.privateKeyDocOrg,
    },
    mspId: config.mspIdDocOrg,
    type: 'X.509',
  };

  await wallet.put(config.mspIdDocOrg, docorgIdentity);
