...
healthRouter.get('/live', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const qsccPatOrg = req.app.locals[config.mspIdPatOrg]
      ?.qsccContract as Contract;
    const qsccDocOrg = req.app.locals[config.mspIdDocOrg]
      ?.qsccContract as Contract;

    await Promise.all([
      getBlockHeight(qsccPatOrg),
      getBlockHeight(qsccDocOrg),
