...
```shell
curl --header "X-Api-Key: ${SAMPLE_APIKEY}" http://localhost:3100/api/assets
```
...
```shell
curl --include --header "X-Api-Key: ${SAMPLE_APIKEY}" --request OPTIONS http://localhost:3100/api/assets/asset7
```

### Create an asset...

```shell
curl --include --header "Content-Type: application/json" --header "X-Api-Key: ${SAMPLE_APIKEY}" --request POST --data '{"ID":"asset7","Color":"red","Size":42,"Owner":"Jean","AppraisedValue":101}' http://localhost:3100/api/assets
```
...
```shell
curl --header "X-Api-Key: ${SAMPLE_APIKEY}" http://localhost:3100/api/jobs/__job_id__
```
...
```shell
curl --header "X-Api-Key: ${SAMPLE_APIKEY}" http://localhost:3100/api/transactions/__transaction_id__
```
...
```shell
curl --header "X-Api-Key: ${SAMPLE_APIKEY}" http://localhost:3100/api/assets/asset7
```
...
```shell
curl --include --header "Content-Type: application/json" --header "X-Api-Key: ${SAMPLE_APIKEY}" --request PUT --data '{"ID":"asset7","Color":"red","Size":11,"Owner":"Jean","AppraisedValue":101}' http://localhost:3100/api/assets/asset7
```

### Transfer an asset...

```shell
curl --include --header "Content-Type: application/json" --header "X-Api-Key: ${SAMPLE_APIKEY}" --request PATCH --data '[{"op":"replace","path":"/Owner","value":"Ashleigh"}]' http://localhost:3100/api/assets/asset7
```

### Delete an asset...

```shell
curl --include --header "X-Api-Key: ${SAMPLE_APIKEY}" --request DELETE http://localhost:3100/api/assets/asset7
```
